pipeline {
    ...
    environment {
        ...
        PORTAINER_DEV_WEBHOOK = credentials('PORTAINER_WEBHOOK_DEV_VSCODE')
        PORTAINER_PRD_WEBHOOK = credentials('PORTAINER_WEBHOOK_PRD_VSCODE')
    }
    ...
    stages {
        ...
        stage('deploy dev code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_dev.yml --extra-vars repo="vscode"'
            }
        }
        ...
        stage('deploy prd code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_prd.yml --extra-vars repo="vscode"'
            }
        }
        // trigger portainer redeploy
        // separated out so this only gets run if the ansible playbook doesn't fail
